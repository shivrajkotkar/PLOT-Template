...
h11 = np.loadtxt("SC",skiprows=1)
h13 = np.loadtxt("BCC",skiprows=1)
h14 = np.loadtxt("FCC",skiprows=1)
...
ax1.errorbar(h11[:,0],h11[:,4],color=c1,lw=lscale*lwidth,linestyle='-',label=r'CS')
ax1.errorbar(h13[:,0],h13[:,4],color=c3,lw=lscale*lwidth,linestyle='-',label=r'BCC')
ax1.errorbar(h14[:,0],h14[:,4],color=c4,lw=lscale*lwidth,linestyle='-',label=r'FCC')
...
ax1.set_yticks([-3.0,-2.5,-2.0,-1.5,-1.0,-0.5,0])

#ax1.set_yscale('log')
ax1.set_ylabel(r'$\Delta \mu^{*}$',labelpad=1)
ax1.set_xlabel(r'$P^{*}$',labelpad=1)
...
legend=ax1.legend(loc="lower left",handlelength=2,numpoints=1,ncol=1,columnspacing=0.5,handletextpad=0.3,labelspacing=0.3) 
...
plt.savefig('Chemical_potential.png',transparent=True)
plt.savefig('Chemical_potential.pdf',transparent=True)
